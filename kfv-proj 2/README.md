...
## 3b. Ground the odds in REAL data (any sport, one command)

The shipped σ values are guesses. To make the odds real, `calibrate.py` scrapes
real historical games from ESPN's public API (no key), fits σ, and checks
calibration — all in one command, for any league:

```bash
python src/calibrate.py --league wnba  --seasons 2023 2024 2025 --write-config
python src/calibrate.py --league nba    --seasons 2023 2024 --write-config
python src/calibrate.py --league ncaamb --seasons 2024 2025 --write-config
python src/calibrate.py --league nfl    --seasons 2022 2023 2024 --write-config
python src/calibrate.py --league ncaaf  --seasons 2023 2024 --write-config
```

Each run prints the fitted σ and the Brier score, saves `output/sigma_<league>.json`
and a `output/calibration_<league>.png` reliability plot, and (`--write-config`)
writes the real σ back into `config.yaml` so every future scan uses it.

Tips:
- First run is slow (it pulls every game day-by-day). Add `--limit 40` for a quick
  test, or fewer `--seasons`.
- Check the calibration plot hugs the diagonal and Brier is well under 0.25 before
  trusting the odds. If a sport calibrates badly, the normal-margin assumption
  doesn't fit it well.
- The lower-level scraper is `build_pbp.py` if you just want the raw CSV.

Then scan with the real σ:
```bash
python src/scan.py --league wnba --sigma output/sigma_wnba.json --watch --interval 30
```
(or just `--league wnba` if you used `--write-config`).
